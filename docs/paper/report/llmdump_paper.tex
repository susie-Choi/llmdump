...
LLMDump 시스템은 데이터 수집과 LLM 분석의 두 계층으로 구성된다.
...
\item XPath injection 수정 커밋을 CVE 등록 전에 탐지하여, LLM 기반 사전 탐지의 효용성을 입증했다.
\end{enumerate}

본 연구의 핵심 기여는 LLM 기반 취약점 탐지가 ``CVE 예측''이라는 좁은 목표를 넘어, 보안 검토가 필요한 코드 영역을 사전에 식별하는 도구로서 실질적 가치가 있음을 보여준 것이다. 향후 대규모 검증과 시간적 검증을 통해 시스템의 효과를 더욱 입증할 계획이다.
